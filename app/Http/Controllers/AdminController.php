...
use App\Models\BodySectionFour;
...
class AdminController extends Controller
{
    ...
    function works(){
        ...
        $d = Works::get();
        $service = Service::get();
        return view('admin.works',compact('data','d','service'));
    }
    ...
    function editWorks($id){
        ...
        $v = Works::find($id);
        $service = Service::get();
        return view('admin.editWorks',compact('data','v','service'));
    ...
    function customer_review(){
        ...
        $d = AddService::latest()->get();
        $service = Service::get();
         $c = CustomerReview::where('status','active')->latest()->get();
        return view('admin.customer_review',compact('data','d','service','c'));
    }
    function addCustomerReview(Request $request){
        $request->validate([
            'name'=>'required',
            'position'=>'required',
        ...
        // if($request->file('image')){
            $image = Image::make($request->file('image'));
            $imageName ='PSC-thumb-'.time().'-'.$request->file('image')->getClientOriginalName();
            $destinationPath = public_path('aminAsset/image/customerReview/');
            $image->resize(55,55);
            $image->save($destinationPath.$imageName);
            $data['image']=$imageName;
        // }
        $r = CustomerReview::create($data);
        if($r){
            return back()->with('success','Customer review added successfully');
        }
    }
    function editCustomerReview($id){
        
        $data = Auth::user();
        $d = CustomerReview::find($id);
        return view('admin.editCustomerReview',compact('data','d'));

    }
    function updateCustomerReview(Request $request, $id){
        $d = CustomerReview::find($id);
        $data = [
            'name'=>$request->name,
            'position'=>$request->position,
        ...
        $r = $d->update($data);
        if($r){
            return redirect()->route('customer_review')->with('success','Customer review updated successfully');
        }
    }
    ...
    function clientCompany(){
        ...
        $d = AddService::latest()->get();
        $service = Service::get();
         $c = ClientCompany::where('status','active')->latest()->get();
        return view('admin.clientCompany',compact('data','d','service','c'));
    }
    function addClients(Request $request){
        ...
        // if($request->file('image')){
            $image = Image::make($request->file('image'));
            $imageName ='PSC-thumb-'.time().'-'.$request->file('image')->getClientOriginalName();
            $destinationPath = public_path('aminAsset/image/clientsLogo/');
            $image->resize(155,155);
            $image->save($destinationPath.$imageName);
            $data['image']=$imageName;
        // }
        $r = ClientCompany::create($data);
        if($r){
            return back()->with('success','Client Company logo added successfully');
        }
    }
    function editcl($id){
        $data = Auth::user();
        $d = AddService::latest()->get();
        $service = Service::get();
         $c = ClientCompany::find($id);
        return view('admin.editcl',compact('data','d','service','c'));
    }
    function updateCDL(Request $request){
        $c = ClientCompany::find($id);
        $data = [
            'title'=>$request->title,
        ...
        $r = $c->update($data);
        if($r){
            return redirect()->route('clientCompany')->with('Success','Client Company Logo Updated Successfully');
        }
    }
    function deletecl($id){
        ...
        $d = $c->delete();
        if($d){
            return back()->with('success','Client Company logo Deleted successfully');
        }
    }
    function whyBest(){
        $data = Auth::user();
        $d = AddService::latest()->get();
        $service = Service::get();
        $b = BodySectionFour::first();
        return view('admin.whyBest',compact('data','d','service','b'));
    }
    function updateBest(Request $r, $id){
        $b = BodySectionFour::find($id);
        $data = [
            'title'=>$r->title,
